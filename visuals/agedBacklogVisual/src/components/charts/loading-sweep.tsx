...
export function LineLoadingSweep({
  ...
  type LoadingSweepPoint = {
    index: number;
    value: number;
  };

  const points: LoadingSweepPoint[] = heights.map((value, index) => ({
    index,
    value,
  }));

  const getX = (d: LoadingSweepPoint) => xScale(d.index);
  const getY = (d: LoadingSweepPoint) => yScale(d.value);
